...
DOCKER_PROJECT_DIR=$(DOCKER_DIR)/$(DOCKER_PROJECT_TYPE)
DOCKER_ENV_FILE:=$(DOCKER_PROJECT_DIR)/.env
DOCKER_COMPOSE_DIR:=$(DOCKER_PROJECT_DIR)/compose
...
.PHONY: set-env
set-env: .docker/network
set-env: .docker/.env ## Docker compose setup environment variables
set-env:
	@echo "Please update $(DOCKER_ENV_FILE) file with your settings"

.PHONY: rm-env
rm-env: ## Remove the .env file for docker compose
...
validate-docker-variables:
	...
	@$(if $(DOCKER_PROJECT_TYPE),,$(error DOCKER_PROJECT_TYPE is undefined - Did you run make-init?))

.docker/.env:
	@cp $(DOCKER_ENV_FILE).example $(DOCKER_ENV_FILE)

.docker/network:
	@docker network ls|grep secure > /dev/null || docker network create --driver bridge secure
...
.PHONY: build
build: docker-build-image ## Build the all other docker images
...
.PHONY: restart
restart: validate-docker-variables ## Restart docker container
	@$(DOCKER_COMPOSE) restart $(DOCKER_SERVICE_NAME)

.PHONY: config
config: validate-docker-variables ## List docker container configuration
